...
def plot_pressure_vs_stretch(results, n_zoom=120): 
    ...
    plt.plot(stretch[:n_zoom], sv_pressure_var[:n_zoom]/1e3, linewidth=2, label='Total')
    plt.plot(stretch[32:n_zoom], sv_pressure_var_elastin[32:n_zoom]/1e3, '--', linewidth=2, label='Elastin')
    plt.plot(stretch[64:n_zoom], sv_pressure_var_collagen[64:n_zoom]/1e3, '-.', linewidth=2, label='Collagen')
    plt.plot(stretch[44:n_zoom], sv_pressure_var_muscle_p[44:n_zoom]/1e3, '--', linewidth=2, label='Muscle Passive')
    plt.plot(stretch[:n_zoom], sv_pressure_var_muscle_a[:n_zoom]/1e3, '--', linewidth=2, label='Muscle Active')
...
def plot_pressure_vs_diameter(results, n_zoom=120): 
    ...
    plt.plot(sv_diam_var[64:n_zoom], sv_pressure_var_collagen[64:n_zoom]/1e3, '-.', linewidth=3, label='Collagen Total')
    plt.plot(sv_diam_var[64:n_zoom], sv_pressure_var_collagen_me[64:n_zoom]/1e3, '-.', linewidth=3, label='Collagen ME')
    plt.plot(sv_diam_var[64:n_zoom], sv_pressure_var_collagen_ad[64:n_zoom]/1e3, '-.', linewidth=3, label='Collagen AD')
    plt.plot(sv_diam_var[44:n_zoom], sv_pressure_var_muscle[44:n_zoom]/1e3, '+', linewidth=2, markersize=8, label='vSMC Total')
    ...
    plt.legend(loc='upper left', fontsize=16, frameon=True, shadow=True, edgecolor='black')
    plt.ylim([0, 60])
...
def plot_stretch_vs_stress(results, n_zoom=120):
    ...
    plt.plot(sv_stretch_var[64:n_zoom], sv_stress_var_muscle[64:n_zoom], '-', linewidth=2, markersize=8, label='vSMC Total')
    plt.plot(sv_stretch_var[44:n_zoom], sv_stress_var_muscle_a[44:n_zoom], '+', linewidth=2, markersize=8, label='aVSMC')
    plt.plot(sv_stretch_var[44:n_zoom], sv_stress_var_muscle_p[44:n_zoom], '.', linewidth=2, markersize=8, label='pVSMC')
    
    plt.plot(sv_stretch_var[:n_zoom], sv_stress_var_total[:n_zoom], '-', linewidth=3, label='Total Stress')    # plt.title('Component-specific Stretch vs. Stress', fontsize=30, weight='bold', pad=25)
...
def plot_stretch_by_geno_treat(results_tt, results_tc, results_cc, 
    ...
    ax[1].legend(fontsize=20, frameon=True, facecolor='white', edgecolor='black', loc='lower right', shadow=True)
...
def plot_stretch_by_score(results_dict, results_dict_treat):
    ...
    for panel_ax, res_dict in panels:
        for i, score in enumerate(sorted(res_dict.keys())):
            ...
            panel_ax.fill_between(time, lower_bound, upper_bound, color=color, alpha=0.15)
            panel_ax.plot(time, center_line, label=labels[score], color=color, linewidth=5.0)
    ...
    ax[1].legend(fontsize=18, frameon=True, facecolor='white', edgecolor='black',
                 loc='lower right', shadow=True, title='Polygenic Scores', title_fontsize=18)
...
def plot_load_bearing_epochs(results_dict, params_dict, epochs=[35, 45, 75]):
    """
...
def plot_time_step_convergence(dt_list, params):
    ...
    ax.plot(dt_plot, ratio_lam, marker='o', linestyle='-', color='black',   linewidth=2, markersize=6, label=r'$\lambda_{sys}$')
    ax.plot(dt_plot, ratio_col, marker='x', linestyle='-', color='#0072B2', linewidth=2, markersize=6, label='Collagen (Adventitia)')
    ax.plot(dt_plot, ratio_tgf, marker='.', linestyle='-', color='#D55E00', linewidth=2, markersize=8, label='Active TGF-β')

    ax.axhline(1.0, color='grey', linestyle=':', linewidth=1.5, alpha=0.6, label='Ratio = 1 (converged)')
