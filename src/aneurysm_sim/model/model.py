...
def simulate_arterial_stress_and_pressure(params):
    ...
    for i in range(n):
        ...
        sv_pressure_var_muscle_p[i] = max(
            functions.pressure_muscle_p(stretch, params), 0
        )
        sv_pressure_var_muscle_a[i] = max(
            functions.pressure_muscle_a(stretch, params), 0
        )
        sv_pressure_var_muscle[i] = (
            sv_pressure_var_muscle_a[i] + sv_pressure_var_muscle_p[i]
        )

        sv_pressure_var[i] = (
            sv_pressure_var_elastin[i]
            + sv_pressure_var_collagen_me[i]
            + sv_pressure_var_collagen_ad[i]
            + sv_pressure_var_muscle[i]
        )
...
def simulate_aneurysm(
    params, treatment=False, dt=0.01
):  # dt in years, step independence achieved at 0.01
    ...
    for i in range(1, steps):
        ...
        latent_tgf_beta[i] = latent_tgf_beta[
            i - 1
        ] + dt * functions.d_latent_tgf_beta_dt(
            active_tgf_beta[i - 1],
            latent_tgf_beta[i - 1],
            fibroblast[i - 1],
            collagen_ad[i - 1],
        ...
        active_tgf_beta[i] = active_tgf_beta[
            i - 1
        ] + dt * functions.d_active_tgf_beta_dt(
            active_tgf_beta[i - 1],
            latent_tgf_beta[i - 1],
            fibroblast[i - 1],
            lambda_c_max[i - 1],
        ...
        alpha = functions.alpha_rate(
            fibroblast[i], collagen_ad[i], collagenase[i], params
        )
        lambda_rec_min[i] = lambda_rec_min[
            i - 1
        ] + dt * functions.d_collagen_min_recruitment_stretch_ad_dt(
            alpha, lambda_c_max[i - 1], lambda_att_max[i - 1]
        )
        lambda_rec_max[i] = lambda_rec_max[
            i - 1
        ] + dt * functions.d_collagen_max_recruitment_stretch_ad_dt(
            alpha, lambda_c_min[i - 1], lambda_att_min[i - 1]
        )
        lambda_rec_mode[i] = lambda_rec_mode[
            i - 1
        ] + dt * functions.d_collagen_mode_recruitment_stretch_ad_dt(
        ...
        lambda_att_max[i] = functions.calculate_max_attachment_stretch(
            lambd_c_max_history, dt, i, params
        )
        lambda_att_min[i] = functions.calculate_min_attachment_stretch(
            lambda_att_max[i], params
        )
        lambda_att_mode[i] = functions.calculate_mode_attachment_stretch(
            lambda_att_min[i], lambda_att_max[i], params
        )
