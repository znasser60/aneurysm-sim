...
def fit_media_stress_hgo_params(params: ArterialParameters = None, show_plot: bool = False):
    """
    ...
    def hgo_stress(lam, k1, k2):
        ...
        fiber_coeff = kappa * (lam**2 - lambda_r**2) + (1.0 - 3.0 * kappa) * (lam**2) * np.cos(gamma)**2
        sigma_fiber = 4.0 * k1 * E * np.exp(k2 * E**2) * fiber_coeff
        
        return sigma_matrix + sigma_fiber

    popt, _ = opt.curve_fit(
        hgo_stress, stretches, target_stress_MPa,
        p0=[0.1, 0.01],
    ...
    if show_plot:
        ...
        r2 = 1 - np.sum((target_stress_MPa - fitted)**2) / np.sum((target_stress_MPa - np.mean(target_stress_MPa))**2)
        print(f"Media — c: {c_init:.6f} MPa, k1: {k1:.6f} MPa, k2: {k2:.6f}, R²: {r2:.4f}")
        print(f"  kappa: {kappa_media}, gamma: {gamma_media}°")

        plt.figure(figsize=(9, 6))
        plt.plot(stretches, target_stress_MPa, 'b-', lw=3, label='0D Model (MPa)')
        plt.plot(stretches, fitted, 'r--', lw=2, label=f'HGO Fit (c={c_init:.4f}, k1={k1:.4f}, k2={k2:.4f})')
        plt.title('Media Cauchy Stress — Effective 1D HGO Model', fontsize=14, weight='bold')
...
def fit_adventitia_stress_hgo_params(params: ArterialParameters = None, show_plot: bool = False):
    """
    ...
    target_stress_Pa  = np.array([functions.sigma_collagen_ad(lam, params) for lam in stretches])
    target_stress_MPa = target_stress_Pa / 1e6
    c_ad = 0.001
    ...
    if show_plot:
        fitted = hgo_stress(stretches, k1, k2)
        r2 = 1 - np.sum((target_stress_MPa - fitted)**2) / np.sum((target_stress_MPa - np.mean(target_stress_MPa))**2)
        print(f"Adventitia — c: {c_ad:.6f} MPa, k1: {k1:.6f} MPa, k2: {k2:.6f}, R²: {r2:.4f}")
        print(f"  kappa: {kappa_ad}, gamma: {gamma_ad}°")

        plt.figure(figsize=(9, 6))
        plt.plot(stretches, target_stress_MPa, 'b-', lw=3, label='0D Model (MPa)')
        plt.plot(stretches, fitted, 'r--', lw=2, label=f'HGO Fit (c={c_ad:.4f}, k1={k1:.4f}, k2={k2:.4f})')
        plt.title('Adventitia Cauchy Stress — Effective 1D HGO Model', fontsize=14, weight='bold')
        plt.xlabel(r'Circumferential Stretch ($\lambda_\theta$)')
        plt.ylabel('Effective Stress (MPa)')
        plt.legend()
        plt.tight_layout()
